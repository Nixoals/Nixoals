<div align="center">
  <img alt="banner" src="./banner.gif" width=900>
</div>

# ABOUT ME

:wave: Hi, my name is Nicolas. I am a tech anthusiast and passionated about science and programming.

After a P.h.D and several years as a research ingeneer in geochemistry, I decided to deep dive into this new digital era to become a web developer. To do so, I started to learn python, javascript, HTML, CSS by myself. I then embarked for a 10 weeks coding bootcamp at LE REACTEUR to strengthen my coding skills. I am now open to work and ready for challenging projects.

- :email: Feel free to contact me nicolasgodeau1@gmail.com
- :globe_with_meridians: Visit my [web page](https://nicolas-godeau-dev.netlify.app/)

# RECENT PROJECTS

## Next Demos

This platform is a social media to share your ideas, build new laws and vote for laws published by the french parliament

<div>
  <img alt="logo next" src="https://res.cloudinary.com/dbrbme99g/image/upload/v1698085785/portfolio/portfolio%20websites/stack/nextjs_prtaia.jpg" height=40>
  <img alt="logo react" src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/512px-React-icon.svg.png?20220125121207" height=40>
  <img alt="logo tailwind" src="https://res.cloudinary.com/dbrbme99g/image/upload/v1698088832/portfolio/portfolio%20websites/stack/tailwind_dcbeiy.png" height=40>
  <img alt="logo nodejs" src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/d9/Node.js_logo.svg/590px-Node.js_logo.svg.png" height=40>
  <img alt="logo mongodb" src="https://res.cloudinary.com/dbrbme99g/image/upload/v1673352611/portfolio/portfolio%20websites/stack/mongodb_ubykjs.jpg" height=40>
  <img alt="logo python" src="https://res.cloudinary.com/dbrbme99g/image/upload/v1698085894/portfolio/portfolio%20websites/stack/python_cvl5h7.png" height=40>
</div>

### Features

- Signin/login Screens
- Social media features (like, comment, share, follow, etc...)
- Automatic data scrapping and parsing
- Automatic data publishing
- NLP with spaCy
- Docker and git for deployment
- see it in [live](https://www.next-demos.fr/)
  
<div align="center">
  <img alt="motto app" src="https://res.cloudinary.com/dbrbme99g/image/upload/v1698085317/portfolio/portfolio%20websites/next-demos/2_tp5ean.png" width=500>
</div>

## Motto-paris app

MVP of a quality vintage clothing app made with React Native.

<div>
  <img alt="logo react" src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/512px-React-icon.svg.png?20220125121207" height=40>
  <img alt="logo nodejs" src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/d9/Node.js_logo.svg/590px-Node.js_logo.svg.png" height=40>
  <img alt="logo threeJS" src="https://upload.wikimedia.org/wikipedia/fr/thumb/4/45/MongoDB-Logo.svg/527px-MongoDB-Logo.svg.png?20190421175613" height=40>
  <img alt="forest admin logo" src="https://res.cloudinary.com/dbrbme99g/image/upload/v1672838906/github/landing%20page/forest_admin_logo_waaiwu.png" height=40>
</div>

### Features

- Signin/login screen with email verification
- Market place of vintage clothes with multiples filters (price, era, brand, size)
- Locker with all the pieces you own, your favorites ones and personnal user information...
- Stripe payment and wallet account
- Full customizable backoffice (forest admin)
- MongoDB database

<div align="center">
  <img alt="motto app" src="https://res.cloudinary.com/dbrbme99g/image/upload/v1672838548/github/landing%20page/motto_image_github_Plan_de_travail_1_jwzf0h.png" width=500>
</div>

## Marvel fan website

- Repository frontend: https://github.com/Nixoals/frontend-marvel-public
- Repository backend: https://github.com/Nixoals/backend-marvel
- see it in [live](https://nixoals-marvel-react.netlify.app/)

<div>
  <img alt="logo react" src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/512px-React-icon.svg.png?20220125121207" height=40>
  <img alt="logo nodejs" src="https://upload.wikimedia.org/wikipedia/commons/thumb/d/d9/Node.js_logo.svg/590px-Node.js_logo.svg.png" height=40>
  <img alt="logo threeJS" src="https://upload.wikimedia.org/wikipedia/fr/thumb/4/45/MongoDB-Logo.svg/527px-MongoDB-Logo.svg.png?20190421175613" height=40>
</div>

<div align="center">
  <img alt="marvelgiv" src="https://res.cloudinary.com/dbrbme99g/image/upload/v1671483965/marvel/screenshots/1_xlx25n.png" width=900>
</div>

---

## My personnal page in 3D

- see it in [live](https://my-room-model.vercel.app/)
- Repository frontend : https://github.com/Nixoals/NicolasGodeau-CV

<div>
  <img alt="logo threeJS" src="https://upload.wikimedia.org/wikipedia/commons/thumb/3/3f/Three.js_Icon.svg/512px-Three.js_Icon.svg.png?20211115112438" height=40>
  <img alt="logo GSAP" src="https://greensock.com/uploads/monthly_2020_03/tweenmax.png.cf27916e926fbb328ff214f66b4c8429.png" height=40>
  
</div>

<div align="center">
  <img alt="my room in 3d" src="./myroom.gif" width=900>
</div>
